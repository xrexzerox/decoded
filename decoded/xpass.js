...
// Provider: XPass
// Original: TMDB/IMDB ID-based streaming
// ============================================

const TMDB_API_KEY = '439c478a771f35c05022f9feabcca01c';
const BASE_URL = 'https://xpassapi.xyz';
const USER_AGENT = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36';

async function getStreams(tmdbId, type = 'movie', season = null, episode = null) {
  console.log('[XPass] Fetching: ' + tmdbId + ' type=' + type);
  try {
    let url;
    ...
    if (data.sources && Array.isArray(data.sources)) {
      for (const source of data.sources) {
        ...
        streams.push({
          name: 'XPass | ' + quality.toUpperCase(),
          title: '🎬 XPass | ' + quality + '\n🎞️ ' + (source.url.includes('.m3u8') ? 'M3U8' : 'MP4') + ' | ⚡ H.264',
          url: source.url,
          quality: '',
          headers: { 'Referer': 'https://xpass.tv/', 'User-Agent': USER_AGENT }
        });
      }
    }
    return streams;
