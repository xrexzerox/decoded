...
const BASE_URL = 'https://allmovielandapi.xyz';
const USER_AGENT = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36';

async function getStreams(tmdbId, type = 'movie', season = null, episode = null) {
  console.log('[AllMovieLand] Fetching: ' + tmdbId + ' type=' + type);
  try {
    let url;
    ...
    if (data.sources && Array.isArray(data.sources)) {
      for (const source of data.sources) {
        ...
        streams.push({
          name: 'AllMovieLand | ' + quality.toUpperCase(),
          title: '🎬 AllMovieLand | ' + quality + '\n🎞️ ' + (source.url.includes('.m3u8') ? 'M3U8' : 'MP4') + ' | ⚡ H.264',
          url: source.url,
          quality: '',
          headers: { 'Referer': 'https://allmovieland.app/', 'User-Agent': USER_AGENT }
        });
      }
    }
    return streams;
